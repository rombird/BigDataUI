...
class Sgui extends JFrame implements ActionListener, KeyListener {
	...
	Sgui() throws Exception {
		...
		area.setEditable(false); // 편집 불가능하게
		...
		txt1.setBounds(10, 260, 260, 30); // 크기, 위치
		...
		setResizable(false); // 크기조절여부 false
		...
		// 소켓연결-ServerSocket
		// 서버 소켓 생성
		server = new ServerSocket(7000); // 192.168.16.203:7000
		...
		dout = new DataOutputStream(client.getOutputStream()); // 소켓 객체 안에 기본 스트림 -> 보조스트림으로 연결
		din = new DataInputStream(client.getInputStream());
		area.append("[CLIENT] " + client.getInetAddress() + " 접속하셨습니다\n"); // area에 표시
		
		//수신스레드 생성
		ServerRecvThread recv = new ServerRecvThread(din,this); // 수신 스트림, 현재주소(this)
	...
	public void keyPressed(KeyEvent e){
		...
		if(e.getKeyCode()==10) //엔터키 입력 
		{
			try {
				...
				//SERVER -> CLIENT (송신)
				dout.writeUTF(txt1.getText()); // outstream에 입력했던 내용 그대로 전달
				dout.flush();
